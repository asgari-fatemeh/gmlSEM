...
2.  Latent variables have zero mean by default if their mean is not fixed to another value by introducing a constraints. Hence, in the regression models with a latent variable does not have an intercept by default. On the other hand, *gmlSEM* includes intercept in the regression model with observed endogenous variables as response. You can change the default by explicitly including or excluding the intercept in a regression model.

3.  As in *lavaan*, you can use label(.), equal(.), and start(.) modifiers with pre-multiplication mechanism. you can also use any other functions which gives a sensible scalar or vector result (for multiple group analysis).

4.  You can extend the family of distributions in *gmlSEM* to fit your needs. However at this stage, you have to define your new family of distributions in the *c language*. To learn how you can define your own family of distribution and use it in *gmlSEM* see `?gmlSEM::extend`.
