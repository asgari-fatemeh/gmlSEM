...
# The family must be aware of that, and to evaluate the params run time
...
#Tipical family
#Support and params can be fixed objects of 'doman' class,
# or functions with no argument returns an object of 'support' depends on other parameters 
# on.args.change is a function that will react to change in the family specific arguments
# This functions if of signature function(...){} which can read and write internal parameters and family specific parameters. The arguments which has changed will be passed to the argument
newFamily<-function(fname,
                    dim=1,  #dimension of probability distribution
                    support=NULL, 
                    params=NULL,
                    link.mean=NULL,
                    link.scale=NULL,
  ...
  extend2<-function(gmlSEMfamily.obj,...){
    ...
    if("link.mean"%in%lst.names){
      ...
      if(!link.mean%in%gmlSEMfamily.obj$link.mean)
        stop("gmlSEM error: invalid link function provided for family: '",
             gmlSEMfamily.obj$family,"'. Valid link functions are:\n",
    ...
    if("link.scale"%in%lst.names){
      ...
      if(!link.mean%in%gmlSEMfamily.obj$link.scale)
        stop("gmlSEM error: invalid link.scale function provided for family: '",
             gmlSEMfamily.obj$family,"'. Valid link functions for parameter 'scale' are:\n",
    ...
    #   stop("gmlSEM error: you can not pass call to the the distribution family '",gmlSEMfamily.obj$family,"' arg(s):",lst1)
    # }
  ...
  support2<-function(gmlSEMfamily.obj){
    ...
    if(!is.null(gmlSEMfamily.obj$computed$support)){
      ...
      stop("Bad support specification in family: ",gmlSEMfamily.obj$family)
    }
  }
  ...
  params2<-function(gmlSEMfamily.obj,par=NA){
    ...
    if(!is.null(gmlSEMfamily.obj$computed$params)){
      ...
      stop("Bad params specification in family: ",gmlSEMfamily.obj$family)
    }
  ...
  fn<-list(#internal slots
           ...
           env                = new_environment(), #for resolving symbols
           names.env          = new_environment(), #for storing runtime params wit class 'name'
           ...
           inflated      = FALSE       ,
           ...                         , 
           #... are specific family parameters such as include_zero in geom() disrtribution, or
  ...
  fn$finite.discrete.support<-function(fam){
    ...
    if(!fam$has.finite.discrete.support(fam))
      stop("gmlSEM error: Family '",fam$family,"' does not have finite discrete support")
